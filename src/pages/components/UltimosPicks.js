...
function UltimosPicks({partidasData, invocadoresData, setIsLoading  }) {
	...
	useEffect(() => {
		const fetchData = async () => {
		  if (partidasData && invocadoresData) {
			try {
			  const response = await fetch('/api/procesarUltimosPicks', {
				method: 'POST',
				headers: {
	...
	const columns = React.useMemo(
        ...
        sortType: (rowA, rowB, columnId, desc) => {
          ...
          return 0;
			...
			{
				Header: 'ULTIMO PICK',
				accessor: 'ultimoCampeon',
        sortType: (rowA, rowB, columnId, desc) => {
          // Obtiene los valores de string de las filas para ordenar
          const valA = rowA.original.ultimoCampeon.toLowerCase();
          const valB = rowB.original.ultimoCampeon.toLowerCase();
